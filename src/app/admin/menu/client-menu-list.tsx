        ...
        <div className="flex gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 w-64 bg-surface border border-surface-border rounded-xl text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-foreground shadow-sm"
            />
          </div>
          <button onClick={handleAddProduct} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl text-sm font-bold shadow-sm hover:opacity-90 transition-opacity">
      ...
      <div className="space-y-8">
        {categories.map((category) => (
          <div key={category.id} className="bg-surface border border-surface-border rounded-2xl overflow-hidden shadow-sm">
            <div className="bg-surface-variant px-6 py-4 border-b border-surface-border flex justify-between items-center">
              <h2 className="font-bold text-foreground text-lg">{category.name}</h2>
            ...
            <div className="divide-y divide-surface-border">
              {category.products.length === 0 ? (
                ...
                category.products.map((product: any) => (
                  <div key={product.id} className={`px-6 py-4 flex items-center justify-between hover:bg-surface-variant/50 transition-colors ${!product.isAvailable ? 'opacity-60' : ''}`}>
